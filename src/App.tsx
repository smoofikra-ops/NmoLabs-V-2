...
import { COMMON_SCHEMAS, generateWebPageSchema, siteMetadata } from './lib/schemas';
...
const WhatToBuild = lazy(() => import('./components/WhatToBuild').then(module => ({ default: module.WhatToBuild })));
...
const MainContent = () => {
  ...
  const renderSection = (id: string) => {
    ...
    switch (id) {
      ...
      case 'productsPreview': return <Suspense key={id} fallback={<div className="h-96" />}><ProductsPreview /></Suspense>;
...
function AppContent() {
  ...
  const schemas = [
    ...
    COMMON_SCHEMAS.erp,
    COMMON_SCHEMAS.crm,
    COMMON_SCHEMAS.ecommerce,
    COMMON_SCHEMAS.webDev,
    COMMON_SCHEMAS.apps
  ];

  return (
    <div className="relative min-h-screen selection:bg-[var(--color-secondary)] selection:text-white pb-20 transition-colors duration-300">
      ...
      ) : config.currentRoute?.startsWith('products/') ? (
        <Suspense fallback={<div className="min-h-screen flex items-center justify-center pt-24"><div className="w-10 h-10 border-4 border-[var(--color-primary)] border-t-transparent rounded-full animate-spin"></div></div>}>
