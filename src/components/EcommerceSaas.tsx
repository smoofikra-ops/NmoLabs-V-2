...
const packages = [
  {
    ...
    border: 'border-[var(--border-default)] hover:border-blue-500/50 hover:shadow-[0_0_30px_rgba(59,130,246,0.3)]',
  ...
  },
  {
    ...
    cta: 'ابدأ النمو',
    popular: true,
    color: 'from-[var(--color-secondary)]/30 to-[var(--color-primary)]/10',
    border: 'border-[var(--color-secondary)]',
    buttonColor: 'bg-gradient-to-r from-[var(--color-primary)] to-[var(--color-secondary)] text-[var(--text-primary)] shadow-[0_0_20px_rgba(124,58,237,0.5)] animate-pulse-slow hover:shadow-[0_0_35px_rgba(124,58,237,0.8)]'
  },
  {
    id: 'full-ops',
    name: 'التشغيل الكامل',
    desc: 'إدارة وتشغيل وتطوير متجرك بالكامل لتحقيق نمو حقيقي ومستدام.',
    icon: <Settings className="w-6 h-6 text-[var(--color-accent)]" />,
    features: [
    ...
    border: 'border-[var(--border-default)] hover:border-[var(--color-accent)]/50 hover:shadow-[0_0_30px_rgba(16,185,129,0.3)]',
    buttonColor: 'bg-[var(--color-accent)]/10 text-[var(--color-accent)] border-[var(--color-accent)]/20 hover:bg-[var(--color-accent)]/30 shadow-[0_0_15px_rgba(16,185,129,0.4)]'
  },
  {
    id: 'custom',
    name: 'الباقة المخصصة',
    ...
    border: 'border-[var(--border-default)] hover:border-orange-500/50 hover:shadow-[0_0_30px_rgba(249,115,22,0.3)]',
...
export const EcommerceSaas = () => {
  ...
  return (
    <section className="py-32 relative bg-[#07070F] overflow-hidden" id="ecommerce">
      {/* Background Effects */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[1000px] h-[500px] bg-[var(--color-secondary)]/10 rounded-full blur-[120px] opacity-50 pointer-events-none"></div>
      <div className="absolute bottom-0 right-0 w-[800px] h-[800px] bg-[var(--color-primary)]/10 rounded-full blur-[150px] opacity-30 pointer-events-none"></div>
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        >
          ...
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-black mb-8 leading-tight font-tajawal">
            ...
            <span className="text-transparent bg-clip-text bg-gradient-to-l from-[var(--color-primary)] via-[var(--color-secondary)] to-[var(--color-primary)] animate-gradient bg-300%">
          ...
          {packages.map((pkg, i) => (
            <motion.div
              ...
              className={`relative rounded-3xl p-8 lg:p-6 xl:p-8 flex flex-col h-full transition-all duration-500
                bg-[var(--surface-secondary)] backdrop-blur-xl border ${pkg.border}
                ${pkg.popular ? 'shadow-[0_0_40px_rgba(124,58,237,0.15)] transform md:-translate-y-4' : 'shadow-md'}
            ...
            >
              ...
              {pkg.popular && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 px-4 py-1.5 bg-gradient-to-r from-[var(--color-primary)] to-[var(--color-secondary)] text-[var(--text-primary)] text-sm font-bold rounded-full shadow-lg z-10 whitespace-nowrap">
                  {pkg.badge}
                </div>
              )}

              <div className="relative z-10 flex flex-col h-full">
                <div className="bg-[var(--surface-primary)]/30 w-14 h-14 rounded-2xl flex items-center justify-center border border-[var(--border-default)] mb-6">
                  {pkg.icon}
                </div>
                
                <h3 className="text-2xl font-bold mb-3">{pkg.name}</h3>
                <p className="text-sm text-[var(--text-muted)] mb-8 min-h-[60px]">{pkg.desc}</p>
                
                <div className="h-px w-full bg-gradient-to-r from-transparent via-[var(--border-default)] to-transparent mb-8"></div>
                ...
                <ul className="space-y-4 flex-grow mb-8 overflow-y-auto pr-2 custom-scrollbar">
                  {pkg.features.map((feat, idx) => (
                    <li key={idx} className="flex items-start gap-3 text-sm text-[var(--text-secondary)]">
                      ...
                      <span className={feat.includes('+') ? 'font-bold text-[var(--text-primary)]' : ''}>{feat}</span>
                    </li>
                  ))}
                </ul>
        ...
        <motion.div
           ...
           whileInView={{ opacity: 1, y: 0 }}
           viewport={{ once: true }}
           className="mt-16 mb-32 overflow-x-auto rounded-3xl border border-[var(--border-default)] bg-[var(--surface-secondary)] backdrop-blur-md custom-scrollbar"
        >
          <table className="w-full text-left border-collapse" dir="rtl">
            <thead>
              <tr className="bg-[var(--surface-secondary)] border-b border-[var(--border-default)]">
                <th className="p-6 text-xl font-bold text-[var(--text-primary)] min-w-[200px]">المميزات</th>
                {packages.map(pkg => (
                  <th key={pkg.id} className="p-6 text-center min-w-[150px]">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <span className="font-bold text-[var(--text-primary)] text-lg">{pkg.name}</span>
                    </div>
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {comparisonFeatures.map((feat, idx) => (
                <tr key={idx} className="border-b border-[var(--border-default)] hover:bg-[var(--surface-secondary)] transition-colors">
                  <td className="p-6 text-[var(--text-secondary)] font-medium">{feat.name}</td>
                  {packages.map(pkg => {
                    ...
                    return (
                      <td key={pkg.id} className="p-6 text-center">
                        {typeof value === 'boolean' ? (
                          value ? (
                            <CheckCircle2 className="w-6 h-6 text-[var(--color-accent)] mx-auto" />
                          ) : (
        ...
        >
          <div className="text-center mb-16">
            ...
            <p className="text-[var(--text-muted)]">خطوات واضحة ومدروسة لانطلاقة مثالية</p>
          </div>
          
          <div className="relative">
            {/* Connecting Line */}
            <div className="absolute top-1/2 left-0 right-0 h-1 bg-gradient-to-r from-transparent via-[var(--color-secondary)]/20 to-transparent -translate-y-1/2 hidden md:block"></div>
              ...
              {timeline.map((item, i) => (
                <div key={i} className="relative z-10 flex flex-col items-center w-full md:w-auto">
                  <motion.div 
                    ...
                    className="w-16 h-16 rounded-full bg-[var(--surface-primary)]/60 border border-[var(--color-secondary)]/30 flex items-center justify-center mb-4 shadow-[0_0_20px_rgba(124,58,237,0.1)] hover:shadow-[0_0_30px_rgba(124,58,237,0.3)] hover:border-[var(--color-secondary)] transition-all duration-300"
                  >
                    {item.icon}
                  </motion.div>
                  <span className="text-sm font-bold text-[var(--text-primary)] mb-1">الخطوة {item.step}</span>
                  <span className="text-sm text-[var(--text-muted)] text-center whitespace-nowrap">{item.title}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* Why NmoLabs section */}
        <div className="mt-32 grid md:grid-cols-2 gap-12 items-center bg-[var(--surface-primary)]/20 p-8 md:p-12 rounded-3xl border border-[var(--border-default)] backdrop-blur-md">
          <div>
            <h3 className="text-3xl font-bold mb-6 font-tajawal">لماذا <span className="text-[var(--color-primary)]">NMOLABS</span>؟</h3>
          ...
          <div className="relative">
            <div className="absolute inset-0 bg-gradient-to-r from-[var(--color-primary)] to-[var(--color-secondary)] blur-[60px] opacity-20 rounded-full"></div>
            <div className="relative glass-panel rounded-2xl p-6 border border-[var(--border-default)] flex flex-col gap-4 shadow-2xl">
              <div className="flex justify-between items-center bg-[var(--surface-secondary)] p-4 rounded-xl border border-[var(--border-default)]">
                <span className="text-[var(--text-muted)]">متوسط زيادة المبيعات</span>
                <span className="text-[var(--color-accent)] font-bold text-xl">+145%</span>
              </div>
              <div className="flex justify-between items-center bg-[var(--surface-secondary)] p-4 rounded-xl border border-[var(--border-default)]">
                <span className="text-[var(--text-muted)]">تحسين معدل التحويل</span>
                <span className="text-[var(--color-primary)] font-bold text-xl">+2.4x</span>
              </div>
              <div className="flex justify-between items-center bg-[var(--surface-secondary)] p-4 rounded-xl border border-[var(--border-default)]">
                <span className="text-[var(--text-muted)]">وقت الانطلاق</span>
                <span className="text-[var(--color-secondary)] font-bold text-xl">أسرع بـ 3 أيام</span>
              </div>
            </div>
          </div>
        </div>
