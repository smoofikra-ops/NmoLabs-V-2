...
const PolicyContainer: React.FC<{ title: string, children: React.ReactNode }> = ({ title, children }) => {
  ...
  return (
    <motion.div 
      ...
      exit={{ opacity: 0, y: -20 }}
      className="bg-[var(--surface-secondary)] backdrop-blur-md rounded-3xl p-8 md:p-12 border border-[var(--border-default)]"
    >
      <button 
        onClick={() => updateConfig({ currentRoute: 'home' })}
        className="flex items-center gap-2 text-[var(--color-primary)] mb-8 hover:text-[var(--text-primary)] transition-colors"
      >
        <ArrowRight size={20} />
        العودة للرئيسية
      </button>
      <h1 className="text-3xl md:text-5xl font-black mb-8 font-tajawal">{title}</h1>
      <div className="prose prose-invert max-w-none prose-headings:text-[var(--color-secondary)] prose-headings:font-bold prose-a:text-[var(--color-primary)] prose-p:text-[var(--text-secondary)]">
        {children}
      </div>
    </motion.div>
  );
