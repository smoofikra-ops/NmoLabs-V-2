...
export const StoryBigVision: React.FC = () => {
  ...
  return (
    <section className="relative py-20 lg:py-28 overflow-hidden bg-[var(--surface-primary)] border-b border-[var(--border-default)] transition-colors duration-300">
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-gradient-to-tr from-[var(--color-primary)]/5 via-[var(--color-secondary)]/5 to-transparent rounded-full blur-3xl pointer-events-none" />

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[var(--surface-secondary)] border border-[var(--border-default)] text-xs font-semibold text-[var(--color-primary)] mb-6 shadow-sm">
            <Sparkles className="w-3.5 h-3.5" />
            <span>{isEn ? 'The Bigger Picture' : 'الرؤية الأكبر'}</span>
          </div>
          ...
          <p className="text-xl sm:text-2xl text-[var(--color-primary)] font-bold mb-8">
            {isEn 
              ? 'NmoLabs is your technology & execution partner from concept to scale.'
              : 'NmoLabs شريكك من الفكرة إلى النمو.'
            }
          </p>

          <p className="text-base sm:text-lg text-[var(--text-muted)] leading-relaxed max-w-2xl mx-auto mb-10">
            {isEn 
          ...
          <div className="flex flex-wrap justify-center gap-2.5 mb-14">
            {(isEn ? AUDIENCES_EN : AUDIENCES).map((audience, idx) => (
              <span 
                key={idx}
                className="text-xs sm:text-sm font-semibold px-4 py-2 rounded-full bg-[var(--surface-secondary)] border border-[var(--border-default)] text-[var(--text-secondary)] shadow-xs"
              >
                {audience}
              </span>
            ))}
          ...
          <div className="p-6 sm:p-8 rounded-3xl border border-[var(--border-default)] bg-[var(--surface-secondary)]/70 backdrop-blur-sm mb-12 shadow-sm">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3 sm:gap-4">
              {LIFECYCLE_STEPS.map((stepItem) => {
                ...
                return (
                  <div 
                    key={stepItem.step}
                    className="p-3 sm:p-4 rounded-2xl bg-[var(--surface-primary)] border border-[var(--border-default)] flex flex-col items-center justify-center text-center shadow-xs"
                  >
