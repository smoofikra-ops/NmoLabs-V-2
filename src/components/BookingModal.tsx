...
export const BookingModal = () => {
  ...
  const getTranslatedText = (key: string) => {
    const trans: Record<string, { ar: string, en: string }> = {
      title: { ar: 'احجز استشارتك المجانية الحين', en: 'Book Your Free Consultation' },
      subtitle: { ar: 'اختار الوقت اللي يناسبك عشان نناقش تفاصيل مشروعك', en: 'Select the best time to discuss your store requirements' },
      fullName: { ar: 'الاسم الكامل', en: 'Full Name' },
      mobileNumber: { ar: 'رقم الجوال (سعودي)', en: 'Saudi Mobile Number' },
      ...
      bookBtn: { ar: 'أكد الحجز وثبت الموعد', en: 'Confirm Booking & Schedule' },
      successTitle: { ar: 'تم جدولة الموعد بنجاح!', en: 'Booking Confirmed Successfully!' },
      successMsg: { ar: 'حفظنا بياناتك ومستشارينا بيكلمونك في وقت الموعد إن شاء الله.', en: 'Your consultation request has been stored. A growth advisor will reach out during your selected time.' },
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[var(--surface-primary)/80] backdrop-blur-md" dir={isRTL ? 'rtl' : 'ltr'}>
        ...
        >
          ...
          <div className="flex items-center justify-between p-6 border-b border-[var(--border-default)] bg-[var(--surface-primary)]/20">
            <div>
              <h3 className="text-xl md:text-2xl font-bold text-[var(--text-primary)] flex items-center gap-2">
                <CalendarIcon className="w-6 h-6 text-[var(--color-primary)]" />
                {getTranslatedText('title')}
              </h3>
              <p className="text-xs text-[var(--text-muted)] mt-1">{getTranslatedText('subtitle')}</p>
            </div>
            <button
              onClick={() => setIsOpen(false)}
              className="p-2 rounded-full hover:bg-[var(--surface-tertiary)] transition-all text-[var(--text-muted)] hover:text-[var(--text-primary)]"
            >
              <X size={20} />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 scrollbar-thin">
            {isSuccess ? (
              ...
              >
                ...
                <h4 className="text-2xl font-bold text-[var(--text-primary)] mb-2">{getTranslatedText('successTitle')}</h4>
                <p className="text-[var(--text-muted)] max-w-md leading-relaxed mb-6 text-sm">{getTranslatedText('successMsg')}</p>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-medium text-[var(--text-muted)] mb-2 flex items-center gap-1.5">
                      ...
                      <button
                        ...
                        className={`py-3 rounded-xl border text-sm font-semibold transition-all ${
                          clientType === 'individual'
                            ? 'bg-[var(--color-primary)]/15 border-[var(--color-primary)]/50 text-[color:var(--color-brand-green-val)]'
                            : 'bg-[var(--surface-secondary)] border-[var(--border-default)] text-[var(--text-muted)] hover:bg-[var(--surface-primary)]/60'
                        }`}
                      >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-1.5 max-h-24 overflow-y-auto p-1 bg-[var(--surface-secondary)] rounded-xl border border-[var(--border-default)] scrollbar-thin">
                      {timeSlots.map((time) => (
                        <button
                          key={time}
                          type="button"
